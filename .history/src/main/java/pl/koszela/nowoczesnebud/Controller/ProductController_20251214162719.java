...
public class ProductController {
    ...
    public ResponseEntity<List<Product>> importProductsWithNames(
        ...
        try {
            ...
            logger.error("❌ Błąd walidacji podczas importu: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        } catch (OutOfMemoryError e) {
            logger.error("❌ Błąd pamięci podczas importu {} plików: {}", files.length, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INSUFFICIENT_STORAGE).build();
        } catch (IOException e) {
            logger.error("❌ Błąd I/O podczas importu: {}", e.getMessage(), e);
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        } catch (Exception e) {
            logger.error("❌ Nieoczekiwany błąd podczas importu {} plików: {}", files.length, e.getMessage(), e);
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
