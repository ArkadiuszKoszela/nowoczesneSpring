...
public class ProductController {
    ...
    public ResponseEntity<List<Product>> importProductsWithNames(
        ...
        logger.info("⏱️ [PERFORMANCE] [Import cenników] Request START: POST /api/products/import-with-names | kategoria: {} | plików: {}", category, files.length);
        ...
        if (files.length != names.length) {
            logger.error("❌ [PERFORMANCE] [Import cenników] Błąd walidacji: liczba plików ({}) != liczba nazw ({})", files.length, names.length);
            return ResponseEntity.badRequest().build();
        }
        ...
        try {
            ...
            logger.info("⏱️ [PERFORMANCE] [Import cenników] Request END: POST /api/products/import-with-names | kategoria: {} | plików: {} | produktów: {} | czas całkowity: {}ms", 
                       category, files.length, products.size(), requestDuration);
            
            return ResponseEntity.ok(products);
        } catch (IllegalArgumentException e) {
            long requestEndTime = System.currentTimeMillis();
            long requestDuration = requestEndTime - requestStartTime;
            logger.error("❌ [PERFORMANCE] [Import cenników] Request ERROR: POST /api/products/import-with-names | kategoria: {} | plików: {} | czas do błędu: {}ms | błąd: {}", 
                       category, files.length, requestDuration, e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        } catch (OutOfMemoryError e) {
            long requestEndTime = System.currentTimeMillis();
            long requestDuration = requestEndTime - requestStartTime;
            logger.error("❌ [PERFORMANCE] [Import cenników] Request ERROR: POST /api/products/import-with-names | kategoria: {} | plików: {} | czas do błędu: {}ms | błąd: OutOfMemoryError", 
                       category, files.length, requestDuration);
            return ResponseEntity.status(HttpStatus.INSUFFICIENT_STORAGE).build();
        } catch (IOException e) {
            long requestEndTime = System.currentTimeMillis();
            long requestDuration = requestEndTime - requestStartTime;
            logger.error("❌ [PERFORMANCE] [Import cenników] Request ERROR: POST /api/products/import-with-names | kategoria: {} | plików: {} | czas do błędu: {}ms | błąd: IOException", 
                       category, files.length, requestDuration, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        } catch (Exception e) {
            long requestEndTime = System.currentTimeMillis();
            long requestDuration = requestEndTime - requestStartTime;
            logger.error("❌ [PERFORMANCE] [Import cenników] Request ERROR: POST /api/products/import-with-names | kategoria: {} | plików: {} | czas do błędu: {}ms | błąd: {}", 
                       category, files.length, requestDuration, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
