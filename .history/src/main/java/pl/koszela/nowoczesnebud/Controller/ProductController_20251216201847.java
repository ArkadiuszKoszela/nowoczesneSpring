    ...
    public SseEmitter deleteMultipleGroupsWithProgress(@RequestBody Map<String, Object> request) {
        ...
        new Thread(() -> {
            try {
                ...
                finalStatus.put("deletedProducts", 0); // Można dodać licznik jeśli potrzebne
                finalStatus.put("elapsedTime", System.currentTimeMillis() - startTime);
                
                emitter.send(SseEmitter.event()
                    .name("completed")
                    .data(finalStatus));
                
                // Dodatkowo wyślij jako zwykły event "progress" z statusem completed
                emitter.send(SseEmitter.event()
                    .name("progress")
                    .data(finalStatus));
