...
public class CreateOffer {
    ...
    /**
     * Wrapper dla nowej struktury CommercialOffer
     * Konwertuje CommercialOffer -> Offer i wywołuje createOffer()
     */
    public void createOfferFromCommercialOffer(CommercialOffer commercialOffer) {
        // Konwertuj Customer -> User
        User user = new User();
        Customer customer = commercialOffer.getCustomer();
        if (customer != null) {
            user.setName(customer.getName());
            user.setSurname(customer.getSurname());
            user.setEmail(customer.getEmail());
            user.setTelephoneNumber(customer.getTelephoneNumber());
            user.setDateOfMeeting(customer.getDateOfMeeting());
            
            // Konwertuj CustomerAddress -> Address
            if (customer.getAddress() != null) {
                ...
                address.setZoom(customer.getAddress().getZoom());
                user.setAddress(address);
            }
        }

        // Konwertuj OfferItem -> Input
        List<Input> inputList = new ArrayList<>();
        if (commercialOffer.getItems() != null) {
            for (OfferItem item : commercialOffer.getItems()) {
                Input input = new Input();
                input.setName(item.getName());
                input.setMapperName(item.getMapperName());
                input.setQuantity(item.getQuantity());
                inputList.add(input);
            }
        }

        // Utwórz stary obiekt Offer
        Offer offer = new Offer();
        offer.setUser(user);
        offer.setInputList(inputList);
