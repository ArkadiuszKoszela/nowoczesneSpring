 ...
 * 
 * ZAKTUALIZOWANE: Używa danych z Input (snapshot) zamiast Product (cennik)
 * Dzięki temu PDF jest generowany na podstawie zapisanych danych projektu,
 * a nie aktualnego cennika który mógł się zmienić.
 */
@Service
public class CreateOffer {
    ...
    public void createOffer(Project project) {
        ...
        try {
            ...
            // ========== Pobierz Input (snapshoty produktów) z projektu ==========
            List<Input> allInputs = project.getInputs() != null ? project.getInputs() : List.of();
            
            // Filtruj tylko Input z quantity > 0
            allInputs = allInputs.stream()
                    .filter(i -> i.getQuantity() != null && i.getQuantity() > 0)
                    .collect(Collectors.toList());
            
            // Podziel po kategoriach
            List<Input> allTiles = allInputs.stream()
                    .filter(i -> i.getCategory() == ProductCategory.TILE)
                    .collect(Collectors.toList());
            List<Input> allGutters = allInputs.stream()
                    .filter(i -> i.getCategory() == ProductCategory.GUTTER)
                    .collect(Collectors.toList());
            List<Input> allAccessories = allInputs.stream()
                    .filter(i -> i.getCategory() == ProductCategory.ACCESSORY)
                    .collect(Collectors.toList());

            // Podziel produkty na główne i opcjonalne
            // ⚠️ WAŻNE: Tylko Input z isMainOption == true (Główna) lub isMainOption == false (Opcjonalna)
            // Input z isMainOption == null (Nie wybrano) są POMIJANE i NIE trafiają do PDF!
            List<Input> mainTiles = allTiles.stream()
                    .filter(i -> i.getIsMainOption() != null && i.getIsMainOption() == true)
                    .collect(Collectors.toList());
            List<Input> optionalTiles = allTiles.stream()
                    .filter(i -> i.getIsMainOption() != null && i.getIsMainOption() == false)
                    .collect(Collectors.toList());
            
            // Rynny i akcesoria - tylko te które są oznaczone jako główne (true) lub opcjonalne (false)
            // null = Nie wybrano - pomijamy!
            List<Input> mainGutters = allGutters.stream()
                    .filter(i -> i.getIsMainOption() != null && i.getIsMainOption() == true)
                    .collect(Collectors.toList());
            List<Input> optionalGutters = allGutters.stream()
                    .filter(i -> i.getIsMainOption() != null && i.getIsMainOption() == false)
                    .collect(Collectors.toList());
            
            List<Input> mainAccessories = allAccessories.stream()
                    .filter(i -> i.getIsMainOption() != null && i.getIsMainOption() == true)
                    .collect(Collectors.toList());
            List<Input> optionalAccessories = allAccessories.stream()
                    .filter(i -> i.getIsMainOption() != null && i.getIsMainOption() == false)
                    .collect(Collectors.toList());
            ...
            // ========== OFERTY OPCJONALNE ==========
            boolean hasOptionalProducts = !optionalTiles.isEmpty() || !optionalGutters.isEmpty() || !optionalAccessories.isEmpty();
            if (hasOptionalProducts) {
                ...
                // Opcjonalne dachówki - pogrupowane po producencie i grupie
                if (!optionalTiles.isEmpty()) {
                    Map<String, List<Input>> groupedOptionalTiles = optionalTiles.stream()
                        .collect(Collectors.groupingBy(input -> {
                            String manufacturer = input.getManufacturer() != null ? input.getManufacturer() : "Inne";
                            String groupName = input.getGroupName() != null ? input.getGroupName() : "Bez grupy";
                            return manufacturer + " - " + groupName;
                        }));
                    
                    for (Map.Entry<String, List<Input>> entry : groupedOptionalTiles.entrySet()) {
                        String groupKey = entry.getKey();
                        List<Input> groupInputs = entry.getValue();
                        
                        if (!groupInputs.isEmpty()) {
                            String optionTitle = "OPCJA " + optionNumber + ": " + groupKey;
                            addElegantProductSection(document, optionTitle, groupInputs);
                            
                            double optionTotal = calculateTotal(groupInputs, List.of(), List.of());
                ...
                // Opcjonalne rynny
                if (!optionalGutters.isEmpty()) {
                    String optionTitle = "OPCJA " + optionNumber + ": System rynnowy (opcjonalny)";
                    addElegantProductSection(document, optionTitle, optionalGutters);
                    
                    double optionTotal = calculateTotal(List.of(), optionalGutters, List.of());
                    addElegantSummary(document, optionTotal, false);
                ...
                if (!optionalAccessories.isEmpty()) {
                    String optionTitle = "OPCJA " + optionNumber + ": Akcesoria (opcjonalne)";
                    addElegantProductSection(document, optionTitle, optionalAccessories);
                    
                    double optionTotal = calculateTotal(List.of(), List.of(), optionalAccessories);
                    addElegantSummary(document, optionTotal, false);
     ...
     * ZAKTUALIZOWANE: Używa Input zamiast Product
     */
    private void addElegantProductSection(Document document, String sectionTitle, List<Input> inputs) 
        ...
        for (Input input : inputs) {
            double quantity = input.getQuantity() != null ? input.getQuantity() : 0;
            double sellingPrice = input.getSellingPrice() != null ? input.getSellingPrice() : 0;
            double totalValue = quantity * sellingPrice;
            sectionTotal += totalValue;
            
            // Jednostka - domyślnie "szt" (bo Input nie ma pola unit)
            String unit = "szt";
            
            addElegantDataCell(table, input.getName() != null ? input.getName() : "", isEven);
     ...
     * ZAKTUALIZOWANE: Używa Input zamiast Product
     */
    private double calculateTotal(List<Input> tiles, List<Input> gutters, List<Input> accessories) {
        double total = 0;
        for (Input i : tiles) {
            total += (i.getSellingPrice() != null ? i.getSellingPrice() : 0) * (i.getQuantity() != null ? i.getQuantity() : 0);
        }
        for (Input i : gutters) {
            total += (i.getSellingPrice() != null ? i.getSellingPrice() : 0) * (i.getQuantity() != null ? i.getQuantity() : 0);
        }
        for (Input i : accessories) {
            total += (i.getSellingPrice() != null ? i.getSellingPrice() : 0) * (i.getQuantity() != null ? i.getQuantity() : 0);
        }
        return total;
    }
