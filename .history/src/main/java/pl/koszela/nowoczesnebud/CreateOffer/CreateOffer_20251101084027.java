...
public class CreateOffer {
    ...
    private static final BaseColor SUCCESS_COLOR = new BaseColor(34, 197, 94); // Zielony dla podsumowania
    ...
    public void createOffer(Project project) {
        ...
        try {
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(new File(StaticValuesForOffer.URL_TO_PDF)));
            document.open();
            ...
            if (allTiles.isEmpty() && allGutters.isEmpty() && allAccessories.isEmpty()) {
                System.err.println("⚠️ BRAK PRODUKTÓW DO PDF!");
                throw new IllegalStateException("Brak produktów do wygenerowania PDF.");
            }

            // ========== OFERTA GŁÓWNA ==========
            boolean hasMainOffer = !mainTiles.isEmpty() || !allGutters.isEmpty() || !allAccessories.isEmpty();
            if (hasMainOffer) {
                ...
                // Rynny
                if (!allGutters.isEmpty()) {
                    addElegantProductSection(document, "System rynnowy", allGutters);
                    document.add(Chunk.NEWLINE);
                }

                // Akcesoria
                if (!allAccessories.isEmpty()) {
                    addElegantProductSection(document, "Akcesoria", allAccessories);
                    document.add(Chunk.NEWLINE);
                }

                // Podsumowanie oferty głównej
                double mainTotal = calculateTotal(mainTiles, allGutters, allAccessories);
            ...
            if (!optionalTiles.isEmpty()) {
                ...
                addElegantProductSection(document, "Alternatywne dachówki", optionalTiles);
                document.add(Chunk.NEWLINE);
                
                // Podsumowanie opcjonalnych
                double optionalTotal = calculateTotal(optionalTiles, List.of(), List.of());
                addElegantSummary(document, optionalTotal, false);
                document.add(Chunk.NEWLINE);
            }
    ...
    private void addElegantSummary(Document document, double total, boolean isMain) throws DocumentException, IOException {
        ...
        String labelText = isMain ? "WARTOŚĆ OFERTY GŁÓWNEJ:" : "Wartość propozycji opcjonalnej:";
        PdfPCell labelCell = new PdfPCell(new Phrase(labelText, createFont(isMain ? 14 : 12, true, BaseColor.WHITE)));
        ...
        PdfPCell valueCell = new PdfPCell(new Phrase(formatCurrency(total), createFont(isMain ? 18 : 14, true, BaseColor.WHITE)));
