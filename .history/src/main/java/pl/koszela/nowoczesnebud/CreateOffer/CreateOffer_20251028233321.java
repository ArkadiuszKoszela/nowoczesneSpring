...
public class CreateOffer {
    ...
    public void createOffer(Offer offer) {
        ...
        try {
            ...
            System.out.println("📋 Dachówki do PDF: " + tiles.size());
            if (!tiles.isEmpty()) {
            ...
            System.out.println("📋 Rynny do PDF: " + gutters.size());
            if (!gutters.isEmpty()) {
            ...
            System.out.println("📋 Akcesoria do PDF: " + accessories.size());
            if (!accessories.isEmpty()) {
                addProductSection(document, "AKCESORIA", accessories);
                document.add(Chunk.NEWLINE);
            }

            // Sprawdź czy mamy jakiekolwiek produkty
            if (tiles.isEmpty() && gutters.isEmpty() && accessories.isEmpty()) {
                System.err.println("⚠️ BRAK PRODUKTÓW DO PDF!");
                System.err.println("💡 Sprawdź czy:");
                System.err.println("   1. DACHÓWKI: wybrano przynajmniej jedną grupę jako 'Główna' (zielony badge)");
                System.err.println("   2. WSZYSTKIE KATEGORIE: produkty mają ilość > 0");
                System.err.println("   3. Użytkownik kliknął 'Przelicz produkty' w zakładce 'Wprowadź dane'");
                throw new IllegalStateException("Brak produktów do wygenerowania PDF. Dla DACHÓWEK wybierz grupę jako 'Główna'. Dla WSZYSTKICH kategorii upewnij się że produkty mają ilość > 0.");
            }
    ...
    /**
     * Pobierz produkty dla dachówek (TILE) - wymagane isMainOption = true
     */
    private List<Product> getMainProducts(ProductCategory category) {
        ...
        System.out.println("🔍 Sprawdzam produkty dla kategorii: " + category);
        System.out.println("📦 Wszystkich produktów w bazie: " + allProducts.size());
        
        // Dla TILE wymagamy isMainOption = true
        if (category == ProductCategory.TILE) {
            long mainCount = allProducts.stream()
                    .filter(p -> p.getIsMainOption() != null && p.getIsMainOption())
                    .count();
            System.out.println("⭐ Produktów z isMainOption=true: " + mainCount);
            
            long withQuantity = allProducts.stream()
                    .filter(p -> p.getQuantity() != null && p.getQuantity() > 0)
                    .count();
            System.out.println("📊 Produktów z ilością > 0: " + withQuantity);
            
            List<Product> result = allProducts.stream()
                    .filter(p -> {
                        boolean hasMainOption = p.getIsMainOption() != null && p.getIsMainOption();
                        boolean hasQuantity = p.getQuantity() != null && p.getQuantity() > 0;
                        
                        if (hasMainOption && hasQuantity) {
                            System.out.println("  ✅ " + p.getName() + " (qty: " + p.getQuantity() + ", main: " + p.getIsMainOption() + ")");
                        } else {
                            System.out.println("  ❌ " + p.getName() + " (qty: " + p.getQuantity() + ", main: " + p.getIsMainOption() + ")");
                        }
                        
                        return hasMainOption && hasQuantity;
                    })
                    .collect(Collectors.toList());
            
            System.out.println("🎯 Znaleziono produktów TILE do PDF: " + result.size());
            return result;
        } 
        
        // Dla GUTTER i ACCESSORY - tylko quantity > 0 (bez sprawdzania isMainOption)
        else {
            long withQuantity = allProducts.stream()
                    .filter(p -> p.getQuantity() != null && p.getQuantity() > 0)
                    .count();
            ...
            System.out.println("🎯 Znaleziono produktów " + category + " do PDF: " + result.size());
            return result;
        }
    }
