            ...
            public void execute(Connection connection) throws SQLException {
                try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        ...
                        logger.info("⏱️ [PERFORMANCE] Batch {}/{} przetworzony (UPSERT draft_is_main_option) | rekordów: {} | INSERT: {} | UPDATE: {} | SUCCESS_NO_INFO: {} | ERROR: {} | czas: {}ms", 
                                   batchIndex + 1, totalBatches, recordsInBatch, insertCount, updateCount, successNoInfoCount, errorCount, batchUpdateTime);
                        
                        // ⚠️ WAŻNE: Jeśli są błędy, rzuć wyjątek
                        if (errorCount > 0) {
