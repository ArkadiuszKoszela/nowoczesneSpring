...
public class CsvImporterImplTile implements CsvImporter<Tile> {
    ...
    public List<Tile> readAndSaveTilesWithNames(List<MultipartFile> list, List<String> names) throws IOException {
        List<Tile> tilePathList = new ArrayList<>();
        Tile tile = new Tile();
        
        for (int index = 0; index < list.size(); index++) {
            MultipartFile multipartFile = list.get(index);
            String customName = names.get(index);
            ...
            // Użyj nazwy podanej przez użytkownika zamiast parsowania z pliku
            ProductGroup productGroup = new ProductGroup(customName, productTypeList);
            tile.getProductGroupList().add(productGroup);
            ...
            // Sprawdź czy kolejny plik ma tego samego producenta
            boolean hasAnyMore = (index + 1 < list.size()) && 
                getManufacturer(list.get(index + 1).getOriginalFilename())
                .equalsIgnoreCase(getManufacturer(fileName));
    ...
    public List<Gutter> readAndSaveGuttersWithNames(List<MultipartFile> list, List<String> names) throws IOException {
        List<Gutter> gutterPathList = new ArrayList<>();
        Gutter gutter = new Gutter();
        ...
        for (int index = 0; index < list.size(); index++) {
            ...
            gutter.setManufacturer(getManufacturer(fileName));
            // Użyj nazwy podanej przez użytkownika
            ProductGroup productGroup = new ProductGroup(customName, productTypeList);
            gutter.getProductGroupList().add(productGroup);
