    ...
    public void saveDraftChanges(Long projectId, SaveDraftChangesRequest request) {
        ...
        for (int i = 0; i < totalChanges; i++) {
            ...
            if ((i + 1) % batchSize == 0 || (i + 1) == totalChanges) {
                ...
                batchCount++;
                logger.debug("  Batch {}: {} rekordów zapisanych", batchCount, Math.min(batchSize, totalChanges - i));
            }
        }
        
        long insertEndTime = System.currentTimeMillis();
        logger.info("⏱️ [Przelicz produkty] BATCH INSERT (EntityManager): {} rekordów w {} batchach, czas: {}ms", 
                   totalChanges, batchCount, (insertEndTime - insertStartTime));
        
        // ⏱️ PERFORMANCE LOG: Koniec zapisu
        long saveEndTime = System.currentTimeMillis();
        long totalDuration = saveEndTime - saveStartTime;
        logger.info("⏱️ [Przelicz produkty] saveDraftChanges - END: {}ms [DELETE: {}ms, INSERT: {}ms]", 
                   totalDuration, (deleteEndTime - deleteStartTime), (insertEndTime - insertStartTime));
    }
