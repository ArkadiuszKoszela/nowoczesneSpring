...
public class ProjectService {
    ...
    public void saveDraftChanges(Long projectId, SaveDraftChangesRequest request) {
        ...
        // ⚡ OPTYMALIZACJA: Sprawdź, czy to tylko zmiana quantity (bez zmian cen/marży/rabatu)
        // "Przelicz produkty" zmienia głównie quantity, więc możemy użyć szybszego UPDATE zamiast DELETE + INSERT
        boolean isOnlyQuantityChange = request.getCategoryMargin() == null && 
                                       ...
                                       request.getChanges().stream().allMatch(change -> 
                                           change.getDraftRetailPrice() == null &&
                                           change.getDraftPurchasePrice() == null &&
                                           change.getDraftSellingPrice() == null &&
                                           change.getDraftMarginPercent() == null &&
                                           change.getDraftDiscountPercent() == null &&
        ...
        if (isOnlyQuantityChange) {
            ...
            logger.info("⏱️ [PERFORMANCE] Wykryto tylko zmiany quantity - używam UPDATE zamiast DELETE + INSERT");
