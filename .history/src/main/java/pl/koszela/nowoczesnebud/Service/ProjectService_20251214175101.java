...
public class ProjectService {
    ...
    public void saveProjectData(Long projectId, SaveProjectDataRequest request) {
        ...
        if (!allDraftChanges.isEmpty()) {
            ...
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] DB Query: findByCategory (dla productId=0) - {} kategorii w {}ms", 
                       categories.size(), loadProductsByCategoryEndTime - loadProductsByCategoryStartTime);
            ...
            Map<String, Map<String, ProjectDraftChange>> categoryGroupsMap = new HashMap<>();
            for (Map.Entry<String, List<Product>> entry : productsByCategoryMap.entrySet()) {
                String category = entry.getKey();
                List<Product> products = entry.getValue();
                Map<String, ProjectDraftChange> groupsForCategory = new HashMap<>();
                    ...
                    if (p.getManufacturer() != null && p.getGroupName() != null) {
                        ...
                        if (groupDraft != null && groupDraft.getDraftIsMainOption() != null) {
                            ...
                            if (!groupsForCategory.containsKey(groupKey)) {
                                groupsForCategory.put(groupKey, groupDraft);
                            }
                        }
                    }
                }
            ...
            for (ProjectDraftChange draft : allDraftChanges) {
                if (draft.getDraftIsMainOption() != null && draft.getDraftIsMainOption() != GroupOption.NONE) {
                    ...
                    if (draft.getProductId() != null && draft.getProductId() > 0) {
                        ...
                        Map<String, ProjectDraftChange> groupsForCategory = categoryGroupsMap.get(draft.getCategory());
                        if (groupsForCategory != null && !groupsForCategory.isEmpty()) {
                            ...
                            Map.Entry<String, ProjectDraftChange> firstGroup = groupsForCategory.entrySet().iterator().next();
                            groupKey = firstGroup.getKey();
                            ProjectDraftChange groupDraft = firstGroup.getValue();
                            if (groupDraft != null) {
                                // Pobierz manufacturer i groupName z pierwszego produktu w grupie
                                ...
                                if (productsInGroup != null) {
                                    for (Product p : productsInGroup) {
                                        if (p.getManufacturer() != null && p.getGroupName() != null) {
                                            String tempKey = p.getManufacturer() + "_" + p.getGroupName() + "_" + draft.getCategory();
                                            if (tempKey.equals(groupKey)) {
