            ...
            public void execute(Connection connection) throws SQLException {
                try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        ...
                        int processedCount = java.util.Arrays.stream(results).sum();
                        logger.info("⏱️ [PERFORMANCE] Batch {}/{} przetworzony (UPSERT draft_is_main_option) | rekordów: {} | przetworzonych: {} | czas: {}ms", 
                                   batchIndex + 1, totalBatches, recordsInBatch, processedCount, batchUpdateTime);
