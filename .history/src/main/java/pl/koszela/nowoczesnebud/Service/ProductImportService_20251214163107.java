...
public class ProductImportService {
    ...
    public List<Product> importProductsWithCustomNames(
        ...
        logger.info("⏱️ [PERFORMANCE] [Import cenników] ProductImportService.importProductsWithCustomNames - START | kategoria: {} | plików: {}", category, files.size());
        ...
        for (int i = 0; i < files.size(); i++) {
            ...
            long convertDuration = convertEndTime - convertStartTime;
            totalConvertTime += convertDuration;
            ...
            long mappingDuration = mappingEndTime - mappingStartTime;
            totalMappingTime += mappingDuration;
            
            // 3. Parsowanie Excel (Poiji)
            long parseStartTime = System.currentTimeMillis();
            List<Product> productsFromFile = Poiji.fromExcel(mappedFile, Product.class);
            long parseEndTime = System.currentTimeMillis();
            long parseDuration = parseEndTime - parseStartTime;
            totalParseTime += parseDuration;
            
            // 4. Ręczne uzupełnienie rabatów jeśli Poiji ich nie odczytało (problem z typami Integer)
            long fillDiscountsStartTime = System.currentTimeMillis();
            fillDiscountsFromExcel(mappedFile, productsFromFile);
            long fillDiscountsEndTime = System.currentTimeMillis();
            long fillDiscountsDuration = fillDiscountsEndTime - fillDiscountsStartTime;
            totalFillDiscountsTime += fillDiscountsDuration;
            ...
            logger.info("⏱️ [PERFORMANCE] [Import cenników] Plik {}/{}: {} | Konwersja: {}ms | Mapowanie: {}ms | Parsowanie: {}ms | Rabaty: {}ms", 
                       i + 1, files.size(), fileName, convertDuration, mappingDuration, parseDuration, fillDiscountsDuration);
            ...
            logger.info("⏱️ [PERFORMANCE] [Import cenników] Plik {}/{} zakończony: {} | produktów: {} | czas całkowity: {}ms [Konwersja: {}ms, Mapowanie: {}ms, Parsowanie: {}ms, Rabaty: {}ms, Przetwarzanie: {}ms]", 
                       i + 1, files.size(), fileName, productsFromFile.size(), fileDuration, convertDuration, mappingDuration, parseDuration, fillDiscountsDuration, processingDuration);
        ...
        logger.info("⏱️ [PERFORMANCE] [Import cenników] ProductImportService.importProductsWithCustomNames - END | kategoria: {} | plików: {} | produktów: {} | czas całkowity: {}ms [Konwersja: {}ms, Mapowanie: {}ms, Parsowanie: {}ms, Rabaty: {}ms, Przetwarzanie: {}ms]", 
                   category, files.size(), allProducts.size(), totalDuration, totalConvertTime, totalMappingTime, totalParseTime, totalFillDiscountsTime, totalProcessingTime);
