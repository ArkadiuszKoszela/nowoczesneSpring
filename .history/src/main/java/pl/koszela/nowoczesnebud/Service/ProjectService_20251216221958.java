...
public class ProjectService {
    ...
    public void saveProjectData(Long projectId, SaveProjectDataRequest request) {
        ...
        if (request.getProductGroups() != null && !request.getProductGroups().isEmpty()) {
            // ⚡ OPTYMALIZACJA: Użyj JDBC batch insert zamiast Hibernate ORM dla dużej liczby rekordów
            // Hibernate ORM jest bardzo wolny dla 579 rekordów (18.3s), batch insert jest znacznie szybszy
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] PRZED batch insert - {} productGroups do zapisania", request.getProductGroups().size());
            batchInsertProjectProductGroups(projectId, request.getProductGroups());
            long addProductGroupsEndTime = System.currentTimeMillis();
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Batch insert ProjectProductGroup do bazy - {}ms (zapisano {} rekordów)", 
                       addProductGroupsEndTime - addProductGroupsStartTime, request.getProductGroups().size());
        }
        
        // 6. Zapisz tylko zmiany w projekcie (marginesy/rabaty) - bez kolekcji (są już zapisane przez batch insert)
        long finalSaveStartTime = System.currentTimeMillis();
        // ⚡ OPTYMALIZACJA: Używamy entityManager.merge() zamiast projectRepository.save()
        // projectRepository.save() próbuje zapisać wszystkie kolekcje (Input, ProjectProductGroup), co jest wolne
        // entityManager.merge() zapisuje tylko zmiany w samym projekcie (marginesy/rabaty)
        if (!entityManager.contains(project)) {
            project = entityManager.find(Project.class, projectId);
        }
        entityManager.merge(project);
        entityManager.flush(); // Wymuś zapis zmian w projekcie
        long finalSaveEndTime = System.currentTimeMillis();
        logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] DB Save: merge (final - tylko projekt, bez kolekcji) - {}ms", finalSaveEndTime - finalSaveStartTime);
    ...
    /**
     * ⚡ OPTYMALIZACJA: Batch update opcji grupy (draftIsMainOption) dla wielu produktów naraz
     * Aktualizuje TYLKO draft_is_main_option, nie dotyka innych pól
     * Używane gdy użytkownik zmienia wariant oferty dla całej grupy produktów
     * 
     * @param projectId ID projektu
     * @param request Request z listą productId i nową opcją
     */
    @Transactional
    public void updateGroupOptionBatch(Long projectId, pl.koszela.nowoczesnebud.DTO.UpdateGroupOptionBatchRequest request) {
        long startTime = System.currentTimeMillis();
        int totalProductIds = request.getProductIds() != null ? request.getProductIds().size() : 0;
        logger.info("⏱️ [PERFORMANCE] UPDATE GROUP OPTION BATCH - START | projectId: {} | kategoria: {} | produktów: {} | opcja: {}", 
                   projectId, request.getCategory(), totalProductIds, request.getDraftIsMainOption());
        
        if (request.getProductIds() == null || request.getProductIds().isEmpty()) {
            logger.warn("⚠️ Pusta lista productId - nic do zaktualizowania");
            return;
        }
        
        String draftIsMainOptionValue = request.getDraftIsMainOption() != null 
            ? request.getDraftIsMainOption().name() 
            : pl.koszela.nowoczesnebud.Model.GroupOption.NONE.name();
        
        // ⚡ OPTYMALIZACJA: JDBC batch UPSERT - aktualizuj tylko draft_is_main_option
        // Jeśli rekord nie istnieje, utwórz go z tylko draft_is_main_option (pozostałe pola NULL)
        // Jeśli istnieje, zaktualizuj tylko draft_is_main_option
        // ⚠️ WAŻNE: Używamy INSERT ... ON DUPLICATE KEY UPDATE (jak w upsertDraftChanges i saveSingleDraftChange)
        // To pozwala tworzyć nowe rekordy jeśli nie istnieją (jak w saveSingleDraftChange)
        String sql = "INSERT INTO project_draft_changes_ws " +
        ...
        final long[] totalPrepareTime = {0};
        final long[] totalUpdateTime = {0};
        
        // ⚠️ WAŻNE: Upewnij się, że wszystkie poprzednie zmiany są zapisane przed batch update
        entityManager.flush();
        ...
        session.doWork(new Work() {
            ...
            public void execute(Connection connection) throws SQLException {
                try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        int startIndex = batchIndex * batchSize;
                        int endIndex = Math.min(startIndex + batchSize, totalProductIds);
                        int recordsInBatch = endIndex - startIndex;
                        ...
                        for (int i = startIndex; i < endIndex; i++) {
                            ...
                            int paramIndex = 1;
                            // INSERT values (jak w upsertDraftChanges)
                            pstmt.setLong(paramIndex++, projectId);
                        ...
                        // - 1 dla INSERT (nowy rekord)
                        // - 2 dla UPDATE (ON DUPLICATE KEY UPDATE - istniejący rekord został zaktualizowany)
                        ...
                        int errorCount = 0;
                        java.util.Set<Integer> uniqueResults = new java.util.HashSet<>();
                        for (int result : results) {
                            ...
                            if (result == 1) insertCount++;
                                ...
                                // Dodatkowe wartości dodatnie (np. 0 dla no-op) traktujemy jako sukces
                                successNoInfoCount++;
                            }
                        }
                        logger.info("⏱️ [PERFORMANCE] Batch {}/{} przetworzony (UPSERT draft_is_main_option) | rekordów: {} | INSERT: {} | UPDATE: {} | SUCCESS_NO_INFO: {} | ERROR: {} | unikalne wartości: {} | czas: {}ms", 
                                   batchIndex + 1, totalBatches, recordsInBatch, insertCount, updateCount, successNoInfoCount, errorCount, uniqueResults, batchUpdateTime);
                        
                        // ⚠️ WAŻNE: Jeśli są błędy (ujemne wartości inne niż -2), rzuć wyjątek
                        if (errorCount > 0) {
                            logger.error("❌ [PERFORMANCE] Wykryto {} błędów w batchu {}/{}", errorCount, batchIndex + 1, totalBatches);
                            throw new RuntimeException("Błąd podczas batch UPSERT group option: " + errorCount + " błędów");
                        }
                        
                        entityManager.flush();
                    }
                } catch (SQLException e) {
                    logger.error("❌ [PERFORMANCE] Błąd podczas batch UPDATE group option: {}", e.getMessage(), e);
                    throw new RuntimeException("Błąd podczas batch UPDATE group option", e);
                }
            }
        });
        ...
        logger.info("⏱️ [PERFORMANCE] UPDATE GROUP OPTION BATCH - END | produktów: {} | batchy: {} | czas całkowity: {}ms | prepare: {}ms | update: {}ms", 
                   totalProductIds, totalBatches, duration, totalPrepareTime[0], totalUpdateTime[0]);
    }
    ...
    /**
     * ⚡ OPTYMALIZACJA: Batch insert ProjectProductGroup (JDBC batch insert zamiast Hibernate ORM)
     * Znacznie szybsze niż Hibernate ORM dla dużej liczby rekordów (579+)
     * 
     * @param projectId ID projektu
     * @param productGroups Lista productGroups do zapisania
     */
    private void batchInsertProjectProductGroups(Long projectId, List<SaveProjectProductGroupDTO> productGroups) {
        long startTime = System.currentTimeMillis();
        int totalGroups = productGroups.size();
        logger.info("⏱️ [PERFORMANCE] BATCH INSERT PROJECT PRODUCT GROUPS - START | projectId: {} | rekordów: {}", 
                   projectId, totalGroups);
        
        String sql = "INSERT INTO project_product_groups " +
                    "(project_id, category, manufacturer, group_name, is_main_option, created_at) " +
                    "VALUES (?, ?, ?, ?, ?, ?)";
        
        int batchSize = 500; // Mniejsze batche dla productGroups (mniej danych niż ProjectProduct)
        int totalBatches = (int)Math.ceil((double)totalGroups / batchSize);
        ...
        session.doWork(new Work() {
            ...
            public void execute(Connection connection) throws SQLException {
                try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        int startIndex = batchIndex * batchSize;
                        int endIndex = Math.min(startIndex + batchSize, totalGroups);
                        int recordsInBatch = endIndex - startIndex;
                        
                        long batchPrepareStart = System.currentTimeMillis();
                        
                        for (int i = startIndex; i < endIndex; i++) {
                            SaveProjectProductGroupDTO dto = productGroups.get(i);
                            
                            Timestamp now = Timestamp.valueOf(LocalDateTime.now());
                            
                            int paramIndex = 1;
                            pstmt.setLong(paramIndex++, projectId);
                            pstmt.setString(paramIndex++, dto.getCategory().name());
                            pstmt.setString(paramIndex++, dto.getManufacturer());
                            pstmt.setString(paramIndex++, dto.getGroupName());
                            pstmt.setString(paramIndex++, dto.getIsMainOption() != null ? dto.getIsMainOption().name() : GroupOption.NONE.name());
                            pstmt.setTimestamp(paramIndex++, now);
                        ...
                        logger.info("⏱️ [PERFORMANCE] Batch {}/{} zapisany (INSERT ProjectProductGroup) | rekordów: {} | czas zapisu: {}ms | przetworzonych: {}", 
                                   batchIndex + 1, totalBatches, recordsInBatch, batchSaveTime, results.length);
                        
                        entityManager.flush();
                    }
                } catch (SQLException e) {
                    logger.error("❌ [PERFORMANCE] Błąd podczas batch insert ProjectProductGroup: {}", e.getMessage(), e);
                    throw new RuntimeException("Błąd podczas batch insert ProjectProductGroup", e);
                }
            }
        });
        
        long duration = System.currentTimeMillis() - startTime;
        logger.info("⏱️ [PERFORMANCE] BATCH INSERT PROJECT PRODUCT GROUPS - END | rekordów: {} | batchy: {} | czas całkowity: {}ms | prepare: {}ms | save: {}ms", 
                   totalGroups, totalBatches, duration, totalPrepareTime[0], totalSaveTime[0]);
    }
}
