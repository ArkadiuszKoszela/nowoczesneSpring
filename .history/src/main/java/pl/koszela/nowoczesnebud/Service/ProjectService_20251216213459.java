            ...
            public void execute(Connection connection) throws SQLException {
                try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        ...
                        int updatedCount = java.util.Arrays.stream(results).sum();
                        logger.info("⏱️ [PERFORMANCE] Batch {}/{} zaktualizowany (UPDATE draft_is_main_option) | rekordów: {} | zaktualizowano: {} | czas aktualizacji: {}ms", 
                                   batchIndex + 1, totalBatches, recordsInBatch, updatedCount, batchUpdateTime);
