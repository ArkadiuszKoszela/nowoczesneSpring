...
public class TilesService {
    ...
    public List<Tile> getAllTileWithNames(List<MultipartFile> files, List<String> names) throws IOException {
        List<Tile> tiles = csvImporterImplTile.readAndSaveTilesWithNames(files, names);
        if (tiles.size() == 0)
            return tiles;
        deleteAll();
        for (Tile tile : tiles) {
