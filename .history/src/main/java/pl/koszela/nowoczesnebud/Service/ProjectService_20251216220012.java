...
public class ProjectService {
    ...
    public void updateGroupOptionBatch(Long projectId, pl.koszela.nowoczesnebud.DTO.UpdateGroupOptionBatchRequest request) {
        ...
        // ⚠️ WAŻNE: W MySQL 8.0+ VALUES() jest deprecated - używamy aliasu 'new' dla nowych wartości
        String sql = "INSERT INTO project_draft_changes_ws " +
            ...
            public void execute(Connection connection) throws SQLException {
                try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        ...
                        // ⚠️ UWAGA: W MySQL executeBatch() zwraca:
                        // - 1 dla INSERT
                        // - 2 dla UPDATE (ON DUPLICATE KEY UPDATE)
                        // - ujemne wartości dla błędów
                        // Suma może być myląca, więc logujemy tylko liczbę rekordów w batchu
                        int insertCount = 0;
                        int updateCount = 0;
                        ...
                        logger.info("⏱️ [PERFORMANCE] Batch {}/{} przetworzony (UPSERT draft_is_main_option) | rekordów: {} | INSERT: {} | UPDATE: {} | czas: {}ms", 
                                   batchIndex + 1, totalBatches, recordsInBatch, insertCount, updateCount, batchUpdateTime);
