...
public class ProjectService {
    ...
    public void saveProjectData(Long projectId, SaveProjectDataRequest request) {
        ...
        if (!allDraftChanges.isEmpty()) {
            ...
            int findByCategoryCount = 0;
            long findByCategoryTotalTime = 0;
            ...
            for (ProjectDraftChange draft : allDraftChanges) {
                if (draft.getDraftIsMainOption() != null && draft.getDraftIsMainOption() != GroupOption.NONE) {
                    ...
                    if (draft.getProductId() != null && draft.getProductId() > 0) {
                        ...
                        long findByCategoryStartTime = System.currentTimeMillis();
                        List<Product> productsInGroup = productRepository.findByCategory(ProductCategory.valueOf(draft.getCategory()));
                        long findByCategoryEndTime = System.currentTimeMillis();
                        findByCategoryTotalTime += (findByCategoryEndTime - findByCategoryStartTime);
                        findByCategoryCount++;
            ...
            if (findByCategoryCount > 0) {
                logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] findByCategory (productId=0): {} wywołań, łącznie {}ms", 
                           findByCategoryCount, findByCategoryTotalTime);
            }
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Przetworzono {} draft changes z opcjami grup, utworzono {} unikalnych grup", 
                       processedCount, groupOptionsMap.size());
            ...
            if (!groupOptionsMap.isEmpty()) {
                ...
                for (Map.Entry<String, ProjectDraftChange> entry : groupOptionsMap.entrySet()) {
                    ...
                    if (draft.getProductId() != null && draft.getProductId() > 0) {
                        ...
                        // (używamy już wcześniej załadowanych produktów z findByCategory, jeśli były)
                        // To jest rzadki przypadek, więc możemy zostawić jak jest
                        List<Product> productsInGroup = productRepository.findByCategory(ProductCategory.valueOf(draft.getCategory()));
                        for (Product p : productsInGroup) {
                            if (p.getManufacturer() != null && p.getGroupName() != null) {
                                Optional<ProjectDraftChange> groupDraftOpt = allDraftChanges.stream()
                ...
                logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Tworzenie ProjectProductGroup - {}ms", 
                           createGroupsEndTime - createGroupsStartTime);
            }
