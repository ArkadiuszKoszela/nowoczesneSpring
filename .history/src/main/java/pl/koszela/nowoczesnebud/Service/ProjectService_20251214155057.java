...
public class ProjectService {
    ...
    public void saveDraftChanges(Long projectId, SaveDraftChangesRequest request) {
        ...
        // 2. BATCH INSERT - użyj natywnego SQL batch INSERT z JDBC (znacznie szybsze niż saveAll())
        // Problem: saveAll() zajmuje ~40 sekund na 1000 rekordów - Hibernate nie używa efektywnych batch INSERT-ów
        // Rozwiązanie: Użyj natywnego SQL batch INSERT z JDBC PreparedStatement - będzie znacznie szybsze
        long insertStartTime = System.currentTimeMillis();
        logger.info("⏱️ [PERFORMANCE] BATCH INSERT (NATIVE SQL) - START | rekordów: {}", totalChanges);
        // ⚡ OPTYMALIZACJA: Zwiększony batch size do 5000 dla lepszej wydajności
        // Mniejsze batchy (1000) = więcej round-trips do bazy = wolniejsze
        // Większe batchy (5000) = mniej round-trips = szybsze (dla 25575 rekordów: 6 batchy zamiast 26)
        // MySQL obsługuje batchy do ~10000, ale 5000 to bezpieczny kompromis między wydajnością a pamięcią
        int batchSize = 5000; // Batch size dla JDBC - zwiększone z 1000 dla lepszej wydajności
        int totalBatches = (int)Math.ceil((double)totalChanges / batchSize);
        
        String sql = "INSERT INTO project_draft_changes_ws " +
                    "(project_id, product_id, category, draft_retail_price, draft_purchase_price, draft_selling_price, " +
                    "draft_quantity, draft_selected, draft_margin_percent, draft_discount_percent, price_change_source, " +
                    "draft_is_main_option, created_at, updated_at) VALUES " +
                    "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        long totalPrepareTime = 0;
        long totalSaveTime = 0;
        
        // Użyj DataSource bezpośrednio zamiast EntityManager.unwrap() - działa z Spring proxy
        try (Connection connection = dataSource.getConnection();
            ...
            for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                ...
                logger.info("⏱️ [PERFORMANCE] Przygotowanie batcha {}/{} | rekordów: {}", batchIndex + 1, totalBatches, recordsInBatch);
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    // Konwertuj GroupOption na String (używając GroupOptionConverter logiki)
                ...
                logger.info("⏱️ [PERFORMANCE] Batch {}/{} zapisany | rekordów: {} | czas zapisu: {}ms | wstawionych: {}", 
                           batchIndex + 1, totalBatches, recordsInBatch, batchSaveTime, results.length);
            }
        } catch (SQLException e) {
            logger.error("❌ [PERFORMANCE] Błąd podczas batch INSERT: {}", e.getMessage(), e);
            throw new RuntimeException("Błąd podczas batch INSERT draft changes", e);
        }
        
        long prepareTime = totalPrepareTime;
        
        long insertEndTime = System.currentTimeMillis();
        long insertDuration = insertEndTime - insertStartTime;
        logger.info("⏱️ [PERFORMANCE] BATCH INSERT (NATIVE SQL) - END | rekordów: {} | batchy: {} | czas całkowity: {}ms | prepare: {}ms | save: {}ms", 
                   totalChanges, totalBatches, insertDuration, prepareTime, totalSaveTime);
