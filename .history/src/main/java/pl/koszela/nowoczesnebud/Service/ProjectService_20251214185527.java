...
public class ProjectService {
    ...
    public void saveProjectData(Long projectId, SaveProjectDataRequest request) {
        ...
        if (!allDraftChanges.isEmpty()) {
            ...
            long buildMapStartTime = System.currentTimeMillis();
            ...
            long buildMapEndTime = System.currentTimeMillis();
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Budowanie mapy istniejących ProjectProduct - {} produktów w {}ms", 
                       existingProductsMap.size(), buildMapEndTime - buildMapStartTime);
            ...
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Pętla przenoszenia draft changes - {} rekordów w {}ms (nowych: {}, zaktualizowanych: {})", 
                       allDraftChanges.size(), loopEndTime - loopStartTime, newProductsCount, updatedProductsCount);
            
            // ⚡ WAŻNE: Zapisz ProjectProduct do bazy PRZED usunięciem draft changes
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] PRZED flush - ProjectProduct w kolekcji: {}", project.getProjectProducts().size());
            long flushProjectProductsStartTime = System.currentTimeMillis();
            entityManager.flush(); // Zapisuje wszystkie ProjectProduct do bazy
            long flushProjectProductsEndTime = System.currentTimeMillis();
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Flush ProjectProduct do bazy - {}ms", 
                       flushProjectProductsEndTime - flushProjectProductsStartTime);
            
            // Sprawdź, ile ProjectProduct jest teraz w bazie
            long countProjectProductsStartTime = System.currentTimeMillis();
            long projectProductsInDb = projectProductRepository.countByProjectId(projectId);
            long countProjectProductsEndTime = System.currentTimeMillis();
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] ProjectProduct w bazie po flush: {} (sprawdzenie zajęło {}ms)", 
                       projectProductsInDb, countProjectProductsEndTime - countProjectProductsStartTime);
            
            entityManager.clear(); // Czyści cache Hibernate
    ...
    public void saveDraftChanges(Long projectId, SaveDraftChangesRequest request) {
        ...
        // To jest szybsze, bo nie musimy usuwać wszystkich rekordów przed wstawieniem
     ...
     * Znacznie szybsze niż DELETE + INSERT, bo nie usuwa i nie wstawia wszystkich rekordów
     */
    private void updateQuantitiesOnly(Long projectId, SaveDraftChangesRequest request) {
        ...
        String sql = "UPDATE project_draft_changes_ws SET draft_quantity = ?, updated_at = ? " +
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    pstmt.setObject(1, dto.getDraftQuantity());
                    pstmt.setTimestamp(2, Timestamp.valueOf(LocalDateTime.now()));
                    pstmt.setLong(3, projectId);
                    pstmt.setLong(4, dto.getProductId());
                ...
                totalPrepareTime += batchPrepareTime;
                ...
                totalSaveTime += batchSaveTime;
                
                logger.info("⏱️ [PERFORMANCE] Batch {}/{} zaktualizowany | rekordów: {} | czas zapisu: {}ms | zaktualizowanych: {}", 
        ...
        logger.info("⏱️ [PERFORMANCE] UPDATE QUANTITIES ONLY - END | rekordów: {} | batchy: {} | czas całkowity: {}ms | prepare: {}ms | save: {}ms", 
                   totalChanges, totalBatches, duration, totalPrepareTime, totalSaveTime);
    }
