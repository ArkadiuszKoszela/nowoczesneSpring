...
public class ProjectService {
    ...
    public void saveProjectData(Long projectId, SaveProjectDataRequest request) {
        ...
        if (allDraftChanges.isEmpty()) {
            ...
            // Teraz dodaj nowe ProjectProduct z draft changes
            ...
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Pętla przenoszenia draft changes - {} rekordów w {}ms (utworzono {} nowych)", 
                       allDraftChanges.size(), loopEndTime - loopStartTime, newProductsCount);
            ...
            entityManager.flush(); // Zapisuje wszystkie ProjectProduct do bazy natychmiast
            long flushProjectProductsEndTime = System.currentTimeMillis();
            logger.info("⏱️ [PERFORMANCE] [Zapisz projekt] Flush ProjectProduct do bazy - {}ms (zapisano {} rekordów)", 
                       flushProjectProductsEndTime - flushProjectProductsStartTime, project.getProjectProducts().size());
            
            // ⚠️ NIE używamy clear() tutaj, bo potrzebujemy projektu w cache dla dalszych operacji
    ...
    private void upsertDraftChanges(Long projectId, SaveDraftChangesRequest request) {
        ...
        // ⚡ WAŻNE: Używamy connection z EntityManager (w tej samej transakcji Spring)
        ...
        try {
            ...
            // Wyłącz auto-commit dla batch processing (jeśli nie jest już wyłączony przez transakcję)
            if (wasAutoCommit) {
                connection.setAutoCommit(false);
            }
        } catch (SQLException e) {
            logger.warn("⚠️ Nie udało się sprawdzić/ustawić auto-commit: {}", e.getMessage());
        }
        ...
        try {
            ...
            // Przywróć auto-commit jeśli był włączony
            try {
                if (wasAutoCommit && !connection.isClosed()) {
                    connection.setAutoCommit(true);
                }
            } catch (SQLException e) {
                logger.warn("⚠️ Nie udało się przywrócić auto-commit: {}", e.getMessage());
            }
        }
