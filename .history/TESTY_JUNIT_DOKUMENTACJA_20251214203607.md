...
#### OPCJA 1: Użyj tej samej bazy co produkcja (BEZPIECZNE!)

**✅ Rekomendowane dla większości przypadków**

- Wszystkie testy są `@Transactional` → rollback automatyczny
- Nie musisz tworzyć osobnej bazy
- Testujesz na prawdziwych danych

**Konfiguracja:** `application-test-mysql.properties` (domyślnie używa `defaultdb`)

#### OPCJA 2: Utwórz osobną bazę testową (opcjonalne)

**Jeśli chcesz być 100% pewny:**
...
#### Skonfiguruj `application-test-mysql.properties`
...
**Jak to działa:**
```
TEST START → BEGIN TRANSACTION
  ↓
INSERT/UPDATE/DELETE (w transakcji)
  ↓
Sprawdź wyniki
  ↓
ROLLBACK (automatycznie!)
  ↓
TEST END → Baza wraca do stanu sprzed testu
```

**Użytkownik NIE ZOBACZY zmian** - wszystko jest rollbackowane przed zakończeniem testu!
